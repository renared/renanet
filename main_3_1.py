...
rena = renanet.NeuralNet(4,100,100,1)
    ...
    for row in rows:
        if 'N/A' not in row.values() :
            ...
            if np.random.rand()<0.5:
                x.append(int(row['WRank'])-int(row['LRank']))
                x.append(int(row['WPts'])-int(row['LPts']))
                x.append(float(row['AvgW']))
                x.append(float(row['AvgL']))
                c.append(0)
            else :
                x.append(int(row['LRank'])-int(row['WRank']))
                x.append(int(row['LPts'])-int(row['WPts']))
                x.append(float(row['AvgL']))
                x.append(float(row['AvgW']))
                c.append(1)
            X.append(x)
